...
namespace Search {
	...
	public:
		void addDoc(const typename TDoc2::TId& id, const TDoc& doc, const std::vector<std::string>& tokens)
		{
			docs_.insert({id, doc});
		...
		std::optional<std::pair<TDoc, std::vector<std::string>>> findDoc(const typename TDoc2::TId& id) const
		{
			auto ptr = docs_.find(id);
			if( ptr == docs_.end() )
				return std::nullopt;
