...
public:
  ...
  void init(const std::vector<TRes>& all,
            const SearchSettings<typename TRes::TDoc>& sett) {
    tokens_ = &sett.tokens;
...
public:
  ...
  void init(const std::vector<TRes>& all,
    ...
    for (const auto& tk : sett.tokens) {
      ...
      l += tk.size();
      lens_.push_back(l);
      if (lens_.size() + 1 == std::numeric_limits<uint8_t>::max()) {
        // leave last free, later add one to indicate value is calculated
        break;
      }
    }
  }
...
public:
  CompPriorityTextsCallback(
      std::function<void(const typename TRes::TDoc&, std::vector<uint8_t>&)> cb)
      : cb_(cb) {}
  ...
  uint8_t calc(const typename TRes::TDoc& doc,
