...
int main(int argc, char* argv[]) {
  ...
  if (!fs::is_regular_file(test11_input)) {
    ...
    std::cout << "download "
                 ...
                 "simplewiki-latest-pages-articles.xml.bz2 and extract\n";
...
public:
  ...
  Doc() : id_(0) {}
  Doc(uint32_t id, std::string_view title, std::string_view text)
...
void testReadFile(Db<FileStore<Doc>>::BulkWriter& writer, const char* start,
                  const char* end) {
  Doc doc;
  size_t i = 0;
  while (start < end) {
...
void test_index(const fs::path& input, const fs::path& output) {
  ...
  for (size_t i = 0; i < numThreads; ++i) {
    arr[i] = std::async(std::launch::async, testReadFile, std::ref(writers[i]),
                        starts[i], starts[i + 1]);
  }
  for (auto& ft : arr) {
    ft.wait();
...
void test_index_single_thread(const fs::path& input, const fs::path& output) {
  ...
  while (start < end) {
    ...
    auto prc2 = (int)std::round((double)(start - file.data()) /
                                (end - file.data()) * 100);
    if (prc2 != prc) {
      prc = prc2;
      SearchTmp::osyncstream(std::cout) << prc << "\n";
    }
  }
}
