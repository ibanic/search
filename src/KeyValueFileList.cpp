...
void KeyValueFileList::createFile(const fs::path& path, uint64_t tabSize,
  ...
  // num keys
  std::memcpy(file.data() + 5 * sizeof(uint64_t), &numKeys, sizeof(uint64_t));

  file.close();
}

size_t KeyValueFileList::fileSize() const { return file_.size(); }
...
bool KeyValueFileList::isFileVersionOk(const fs::path& pth) {
  ...
  uint64_t ver;
  std::memcpy(&ver, f.data() + 0 * sizeof(uint64_t),
...
std::vector<BytesView>
KeyValueFileList::getWithBucket(uint64_t bucket, std::string_view key) const {
...
bool KeyValueFileList::existsWithBucket(uint64_t bucket, std::string_view key,
...
void KeyValueFileList::setInternal(uint64_t bucket, std::string_view key,
                                   BytesView value) {
...
void KeyValueFileList::removeInternal(uint64_t bucket, std::string_view key,
                                      BytesView value) {
  // find key
...
void KeyValueFileList::bulkWrite(std::ofstream& out, std::string_view key,
                                 BytesView value) {
...
KeyValueFileList::bulkRead(const std::byte*& dt) {
  uint64_t hash;
...
void KeyValueFileList::bulkInsertEnlarge(size_t nthThread, size_t numThreads) {
  ...
  importing_->dataRange[nthThread].second =
      importing_->dataRange[nthThread].first + BulkReserve;
  setNextDataOffset(importing_->dataRange[nthThread].second);
}

void KeyValueFileList::bulkInsert(uint64_t bucket, std::string_view key,
...
void KeyValueFileList::bulkRemove(uint64_t bucket, std::string_view key,
                                  BytesView value, size_t nthThread,
                                  size_t numThreads) {
  std::lock_guard lock1(importing_->mutex_);
...
bool KeyValueFileList::bulkIsInThread(uint64_t bucket, size_t nthThread,
...
uint64_t KeyValueFileList::calcBucketFromHash(uint64_t hash,
                                              uint64_t numBuckets2) {
...
uint64_t KeyValueFileList::numKeys() const {
  uint64_t n;
  std::memcpy(&n, data() + 5 * sizeof(uint64_t), sizeof(n));
  return n;
}
...
void KeyValueFileList::unlockTable() { locked_ = false; }

std::vector<std::pair<std::string_view, BytesView>>
KeyValueFileList::allDocuments() const {
...
void KeyValueFileList::changeTable(uint64_t tabSize, uint64_t contentSize) {
...

  {
    ...
    std::memcpy(buffer2.get() + 4 * sizeof(uint64_t), &numItems,
                sizeof(uint64_t));
    // num keys
    std::memcpy(buffer2.get() + 5 * sizeof(uint64_t), &numKeys,
                sizeof(uint64_t));
  }
...
inline size_t KeyValueFileList::findTabSizePrimeDouble(size_t minNum) {
  ...
  throw std::runtime_error(
      "KeyValueFileList findTabSizePrimeDouble() too big table");
}
