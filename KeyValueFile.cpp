...
namespace Search {
	...
	void KeyValueFile::createFile(const fs::path& path, uint64_t tabSize, uint64_t contentSize)
	{
		// create
		...
		*/
		...
		std::memcpy(file.data()+0*sizeof(uint64_t), &v, sizeof(uint64_t));  // version
	...
	size_t KeyValueFile::fileSize() const
	{
		return file_.size();
	}
	...
	/*void KeyValueFile::setBatch(const std::vector<std::pair<std::string_view, BytesView>>& arr) {
		ensureTableSize(arr.size());
		
		// buckets
		auto buckets = calcBuckets(arr);

		// required space
		size_t newSpace = 0;
		for( const auto& pair : arr ) {
			newSpace += Item::calcSize(pair.first, pair.second);
		}

		ensureFreeSpace(newSpace);

		for( size_t i=0; i<arr.size(); ++i ) {
			setInternal(buckets[i], arr[i].first, arr[i].second);
		}
	...
	void KeyValueFile::setBatch(const std::vector<std::pair<std::string_view, Bytes>>& arr)
	{
		std::vector<std::pair<std::string_view, BytesView>> arr2(arr.size());
		for( size_t i=0; i<arr.size(); ++i ) {
			arr2[i].first = makeStringView(arr[i].first);
			arr2[i].second = makeStringView(arr[i].second);
		}
		return setBatch(arr2);
	}*/
	...
	BytesView KeyValueFile::getWithBucket(uint64_t bucket, std::string_view key) const
	{
	...
	void KeyValueFile::setInternal(uint64_t bucket, std::string_view key, BytesView value)
	{
		auto pair = findInternal(bucket, key);
	...
	std::pair<uint64_t, KeyValueFile::Item> KeyValueFile::findInternal(uint64_t bucket, std::string_view key) const
	{
		...
		while( it.valid() ) {
			...
			it = it.next();
			//assert(prevOffset != it.offset());
		}
		return {0, KeyValueFile::Item()};
	}


	void KeyValueFile::bulkWrite(std::ofstream& out, std::string_view key, BytesView value)
	{
	...
	std::tuple<uint64_t, std::string_view, BytesView> KeyValueFile::bulkRead(const std::byte*& dt)
	{
		uint64_t hash;
	...
	/*Bytes KeyValueFile::getWithBucketImport(uint64_t bucket, std::string_view key) const
	{
		std::lock_guard lock1(importing_->mutex_);
		// make a copy because later it can get destroyed
		auto res = getWithBucket(bucket, key);
	...
	void KeyValueFile::bulkInsertEnlarge(size_t nthThread, size_t numThreads)
	{
		...
		// ensure free space
		size_t reserve = 1'000'000;  // 1Mb
		ensureFreeSpace(reserve);
		importing_->dataRange[nthThread].first = nextDataOffset();
		importing_->dataRange[nthThread].second = importing_->dataRange[nthThread].first + reserve;
		setNextDataOffset(importing_->dataRange[nthThread].second);
	}


	void KeyValueFile::bulkInsert(uint64_t bucket, std::string_view key, BytesView value, size_t nthThread, size_t numThreads)
	{
		auto itemSize = Item::calcSize(key, value);
	...
	void KeyValueFile::bulkRemove(uint64_t bucket, std::string_view key, BytesView value, size_t nthThread, size_t numThreads)
	{
		std::lock_guard lock1(importing_->mutex_);
	...
	bool KeyValueFile::bulkIsInThread(uint64_t bucket, size_t nthThread, size_t numThreads, uint64_t numBuckets2)
	{
		if( numThreads == 1 )
			return true;
	...
	uint64_t KeyValueFile::calcBucket(std::string_view key) const
	{
		...
		/*auto b = CityHash64(key.data(), key.size());
		uint64_t b2 = b % numBuckets();
		return b2;*/
	}


	/*std::vector<uint64_t> KeyValueFile::calcBuckets(const std::vector<std::pair<std::string_view, BytesView>>& arr) const
	{
	...
	void KeyValueFile::unlockTable()
	{
		locked_ = false;
	}


	std::vector<std::pair<std::string_view,BytesView>> KeyValueFile::allDocuments() const
	{
	...
	void KeyValueFile::changeTable(uint64_t tabSize, uint64_t contentSize)
	{
...

		{
			...
			const auto v = boost::endian::native_to_little<uint64_t>(Version);
			std::memcpy(buffer2.get()+0*sizeof(uint64_t), &v, sizeof(uint64_t));  // number of slots in hash table
			std::memcpy(buffer2.get()+1*sizeof(uint64_t), &tabSize, sizeof(uint64_t));  // number of slots in hash table
			std::memcpy(buffer2.get()+2*sizeof(uint64_t), &numWasted, sizeof(uint64_t));  // number of vasted bytes
...
int main()
{
	...
	inline size_t KeyValueFile::findTabSizePrimeDouble(size_t minNum)
	{
		...
		throw std::runtime_error("KeyValueFile findTabSizePrimeDouble() too big table");
	}
